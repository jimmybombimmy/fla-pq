...
export function getPipeRects() {
  return pipes.flatMap(pipe => pipe.rects()) // flatMap turns a multi-dimensional array into a one dimensional array
}
...
function createPipe() {
  ...
  ); // makes sure the hole doesn't appear off the screen or right at the bottom
  const pipe = {
    get left() {
      return parseFloat(
...
function randomNumberBetween(min, max) {
  return Math.floor(Math.random() * (max - min + 1) + min); // gives random number between min and max
}
